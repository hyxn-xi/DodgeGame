...
public class Bullet : MonoBehaviour
{
    ...
    {
        if (other.tag == "Player")          // 충돌한 상대방 게임오브젝트가 Player태그를 가진 경우
        {
            PlayerController playerController = other.GetComponent<PlayerController>();
            // 상대방 게임오브젝트에서 PlayerController 컴포넌트 가져오기

            if(playerController != null)    // 상대방으로부터 PlayerController 컴포넌트 가져오는 데 성공하면
            {
                playerController.Die();     // 상대방 PlayerController 컴포넌트의 Die함수 실행
            }
        }
    }
}
